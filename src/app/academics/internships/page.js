              ...
              >
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <span className="text-[10px] font-black uppercase tracking-wider bg-slate-100 text-[#2B1F1A] px-3 py-1.5 rounded-full border border-slate-200 flex items-center gap-1">
                      <Building2 className="w-3.5 h-3.5 text-[var(--iris)] shrink-0" /> {program.department || "Medical"}
                    </span>
                    <span className="text-[10px] font-black uppercase tracking-wider bg-emerald-100 text-emerald-950 px-3 py-1.5 rounded-full border border-emerald-300 flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-emerald-700 shrink-0" /> {program.duration || "3 Months"}
                    </span>
                  </div>

                  {/* Title */}
                  <h2 className="text-xl font-black text-[#2B1F1A] group-hover:text-[var(--iris)] transition-colors leading-tight">
                    {program.title}
                  </h2>

                  {/* Short Description */}
                  <p className="text-xs sm:text-sm font-extrabold text-slate-900 line-clamp-3 leading-relaxed">
                    {program.description}
                  </p>

                  {/* Program Features / Seats */}
                  <div className="pt-3 border-t border-slate-100 grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-[var(--fog)] p-2.5 rounded-xl border border-slate-200">
                      <span className="text-[10px] font-black uppercase text-slate-800 block">Available Seats</span>
                      <span className="font-black text-[#2B1F1A] flex items-center gap-1 mt-0.5">
                        <Users className="w-3.5 h-3.5 text-[var(--iris)] shrink-0" /> {program.seatsAvailable || "Limited"} Positions
                      </span>
                    </div>
                    <div className="bg-[var(--fog)] p-2.5 rounded-xl border border-slate-200">
                      <span className="text-[10px] font-black uppercase text-slate-800 block">Certificate</span>
                      <span className="font-black text-emerald-950 flex items-center gap-1 mt-0.5">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-700 shrink-0" /> Awarded
                      </span>
                    </div>
                  </div>
                </div>
