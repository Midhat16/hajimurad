  ...
  return (
    <div className="space-y-4 pt-6 border-t border-slate-200">
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            <img
              ...
              alt={currentItem.caption || `Program Photo ${currentIndex + 1}`}
        ...
        <div className="p-4 bg-gradient-to-t from-slate-950/90 via-slate-900/60 to-transparent text-white z-10">
          {currentItem.caption ? (
                ...
                >
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <img
                        src={currentItem.imageUrl}
                        alt={currentItem.caption || "Full photo"}
...
function InternshipDetailsContent() {
  ...
  return (
    <>
      ...
      {/* Main Details Container (Outer card wrapper removed: content displays directly on page background) */}
            ...
            {internship.highlights.some((h) => h.description && h.description.trim()) ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
                {internship.highlights.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-3.5 bg-gradient-to-r from-[#b5cee4] to-[#a0bfd9] p-3.5 rounded-2xl border border-slate-300 shadow-xs hover:shadow-md transition-all group"
                  >
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {internship.quotes.map((q, idx) => (
                <div
                  key={idx}
                  className="bg-gradient-to-br from-blue-50/80 via-white to-blue-100/40 p-4 rounded-2xl border border-blue-200 flex items-center gap-3.5 shadow-xs hover:shadow-md transition-all group"
                >
                  <div className="shrink-0 flex items-center justify-center p-1 rounded-2xl border border-rose-500 bg-white/80 shadow-2xs transition-transform group-hover:scale-105 duration-300 min-w-[44px] min-h-[44px]">
                    <img
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 sm:gap-4">
            {DEFAULT_INFO_BOXES.map((box) => {
              ...
              return (
                <div
                  key={box.id}
                  className={`bg-white hover:bg-gradient-to-br ${box.color} p-4 sm:p-5 rounded-2xl border border-slate-200/90 flex flex-col items-center justify-center text-center space-y-2.5 shadow-xs hover:shadow-md transition-all group`}
                >
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 bg-gradient-to-r from-[var(--ink)] to-[var(--iris)] hover:opacity-95 text-white px-7 py-3.5 rounded-xl text-xs sm:text-sm font-extrabold shadow-lg hover:shadow-xl transition-all cursor-pointer border border-white/20"
            >
