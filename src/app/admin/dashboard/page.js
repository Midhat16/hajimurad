...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    const qMsgsCount = query(collection(db, "messages"), where("read", "==", false));
    const unsubMsgsCount = onSnapshot(
      qMsgsCount,
      (snap) => {
        setCounts((prev) => ({ ...prev, unreadMessages: snap.size }));
      },
      (err) => console.warn("MsgsCount notice:", err.message)
    );

    // 4. Recent Messages list
    const unsubMsgsRecent = onSnapshot(
      collection(db, "messages"),
      (snap) => {
        const list = snap.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() }));
        list.sort((a, b) => (b.createdAt?.toMillis ? b.createdAt.toMillis() : 0) - (a.createdAt?.toMillis ? a.createdAt.toMillis() : 0));
        setRecentMessages(list.slice(0, 5));
      },
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-xs text-[#3F4B4A] font-semibold mt-1">
            ...
            <Link key={idx} href={card.href}>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <ArrowUpRight className="w-5 h-5 text-slate-400 group-hover:text-[#0B3D5C] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
                </div>

                <div>
                  <span className="text-3xl font-black text-[#0B3D5C]">
                    {card.count}
                  </span>
                  <p className="text-xs font-bold text-[#3F4B4A] uppercase tracking-wider mt-1">
                    {card.title}
                  </p>
                </div>
              </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Recent Appointments */}
        <div className="bg-white rounded-3xl border border-[#D5E5DD] p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
              <div className="flex items-center gap-2">
                <Calendar className="w-5 h-5 text-[#3E8E6E]" />
                <h3 className="text-base font-extrabold text-[#0B3D5C]">Recent Appointments</h3>
              </div>
              <Link
                href="/admin/appointments"
                className="text-xs font-bold text-[#3E8E6E] hover:text-[#0B3D5C] flex items-center gap-1"
              >
                View All &rarr;
              </Link>
            </div>
            ...
            {recentAppointments.length === 0 ? (
              ...
              <div className="space-y-3">
                {recentAppointments.map((appt) => (
                  <div
                    key={appt.id}
                    className="p-3.5 rounded-2xl bg-[#F4F7F5] border border-[#D5E5DD]/60 flex items-center justify-between text-xs"
                  >
                    <div>
                      <h4 className="font-bold text-[#0B3D5C]">{appt.name}</h4>
        ...
        <div className="bg-white rounded-3xl border border-[#D5E5DD] p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
              <div className="flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-[#3E8E6E]" />
                <h3 className="text-base font-extrabold text-[#0B3D5C]">Recent Contact Messages</h3>
              </div>
              <Link
                href="/admin/messages"
                className="text-xs font-bold text-[#3E8E6E] hover:text-[#0B3D5C] flex items-center gap-1"
                ...
                {recentMessages.map((msg) => (
                  ...
                  >
                    <div className="min-w-0 pr-2">
                      <h4 className="font-bold text-[#0B3D5C] truncate">{msg.name}</h4>
                    ...
                    {msg.read ? (
                      ...
                      <span className="px-2 py-0.5 rounded-full bg-[#3E8E6E] text-white text-[9px] font-extrabold flex-shrink-0 uppercase">
                        Unread
                      </span>
                    )}
                  </div>
