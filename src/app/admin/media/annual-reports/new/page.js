...
export default function AdminNewAnnualReportPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err?.code === "permission-denied" || err?.message?.includes("permission")) {
        setError("Firebase Permission Error: Please update Firestore Security Rules to allow write access to 'annualReports' collection.");
      } else {
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[var(--line)] shadow-md space-y-6">
        <div>
          ...
          <p className="text-xs font-semibold text-[var(--slate)] mt-0.5">
            Enter period year and upload PDF document from PC or paste cloud link.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="Financial year / Period"
                value={year}
          ...
          <div className="pt-3 flex justify-end">
            ...
            >
              ...
              {isSubmitting ? "Saving Report..." : "Save Annual Report"}
            </button>
          </div>
        </form>
      </div>
