...
function EditReportForm() {
  ...
  useEffect(() => {
    if (!id) {
      router.push("/admin/media/annual-reports");
      return;
    }

    async function fetchReport() {
      try {
        const snap = await getDoc(doc(db, "annualReports", id));
        if (snap.exists()) {
          const data = snap.data();
          setYear(data.year || data.title || "");
          setFileUrl(data.fileUrl || "");
          setOrder(data.order || 1);
        } else {
          setError("Annual Report document not found.");
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    if (!year.trim() || !id) return;

    setIsSubmitting(true);
    try {
      await updateDoc(doc(db, "annualReports", id), {
        title: year.trim(),
        year: year.trim() || "Annual Report",
      ...
      setError("Failed to update annual report.");
    } finally {
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[var(--line)] shadow-md space-y-6">
        <div>
          ...
          <p className="text-xs font-semibold text-[var(--slate)] mt-0.5">
            Update report year/period and PDF document link.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* PDF File Picker */}
          <PdfPicker
            label="Annual Report Document (Upload PDF from PC OR Paste Link) *"
            value={fileUrl}
            onChange={setFileUrl}
            required
          />
          ...
          <div className="pt-3 flex justify-end">
            ...
            >
              ...
              {isSubmitting ? "Updating Report..." : "Update Annual Report"}
            </button>
          </div>
        </form>
      </div>
...
export default function AdminEditAnnualReportPage() {
  return (
    <Suspense
      fallback={
        <div className="flex flex-col items-center justify-center py-20">
