        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
          {doctors.map((doctor, index) => (
            <motion.div
              ...
              className="h-[380px] sm:h-[400px] w-full group cursor-pointer"
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col items-center mt-2">
                    <div className="relative w-24 h-24 rounded-full flex items-center justify-center p-1 bg-gradient-to-tr from-[#0B3D5C]/30 to-[#3E8E6E]/30 shadow-sm overflow-hidden">
                      <DoctorAvatar doctor={doctor} />
                      <div className="absolute -bottom-1 -right-1 bg-white p-1 rounded-full border border-[#D5E5DD] shadow-xs text-[#0B3D5C]">
                        <Stethoscope className="w-3.5 h-3.5" />
                      </div>
                    </div>

                    <h3 className="mt-5 text-xl font-bold text-[#0B3D5C] text-center leading-tight">
                      {doctor.name}
                    </h3>
                    <p className="text-[11px] font-bold text-[#3E8E6E] uppercase tracking-widest mt-1 text-center">
                      {doctor.role}
                    </p>
                    <p className="text-xs text-[#3F4B4A] font-semibold mt-2 text-center">
                      {doctor.specialty}
                    </p>
                  </div>

                  {/* Credentials Footer on Front */}
                  <div className="border-t border-[#D5E5DD]/60 pt-3 flex flex-col gap-2">
                    {doctor.pmdcNo && (
                      <div className="text-[10px] font-bold text-[#0B3D5C] bg-[#E8F0EC] px-2 py-1 rounded-md text-center border border-[#D5E5DD]">
                        PMDC Reg #: <span className="text-[#3E8E6E] font-extrabold">{doctor.pmdcNo}</span>
                      </div>
                    )}
                    {doctor.availabilityDays && (
                      <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-700 justify-center">
                        <Calendar className="w-3.5 h-3.5 text-[#3E8E6E] flex-shrink-0" />
                        <span className="truncate">{doctor.availabilityDays}</span>
                      </div>
                    )}
                    {doctor.education && (
                      <div className="flex items-center gap-2 text-xs font-semibold text-[#3F4B4A]">
                ...
                >
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-[10px] text-[#3E8E6E] font-bold uppercase tracking-wider">{doctor.specialty || doctor.role}</p>
                      {doctor.pmdcNo && (
